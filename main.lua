...
local function webhook(raised, donor)
	...
	local a = {
		...
		["fields"] = {
			...
			{
				["name"] = "После комиссии [ДОНАТ]",
				["value"] = '`' .. '' .. math.floor(tostring(raised * 0.6)) .. '`',
				["inline"] = true
			},
			{
				["name"] = "После комиссии [ВСЕГО]",
...
if game:GetService('CoreGui'):FindFirstChild('RobloxPromptGui') then
	for i, v in next, game:GetService("CoreGui").RobloxPromptGui:GetDescendants() do
		if v:IsA('TextLabel') and string.find(string.lower(v.Text), "ban") then
			oldWebhook('@everyone ||' .. Players.LocalPlayer.Name .. '|| был забанен' .. v.Text:gsub("You've been banned", ''):gsub('You were kicked from this experience:', ''):gsub('(Error Code: 267)', ''):gsub('()',''))
		end
	end
end
...
local Window = library:AddWindow("TpaBa#0001 / RU version",
  {
...
local otherTab2 = Window:AddTab("Другое 2")
...
local noFontSwitch = boothTab:AddSwitch("Отключить шрифт", function(bool)
	if settingsLock then
		return
	end
...
local rainbowSwitch = boothTab:AddSwitch("Радужный шрифт", function(bool)
...
local textUpdateDelay = boothTab:AddSlider("Обновление текста (С)", function(x)
	if settingsLock then
		return
	end
...
boothTab:AddLabel("Кастомный текст таблички:")
...
boothTab:AddButton("Обновить", function()
...
if signPass then
  ...
  --Chat Settings
local autoThanks = chatTab:AddSwitch("Авто сообщения спасибо", function(bool)
	getgenv().settings.autoThanks = bool
	saveSettings()
end)
autoThanks:Set(getgenv().settings.autoThanks)
local autoBeg = chatTab:AddSwitch("Авто сообщения", function(bool)
	if settingsLock then
		return
	end
...
local begDelay = chatTab:AddSlider("Задержка сообщений (С)", function(x)
	if settingsLock then
		return
	end
...
local bm = chatTab:AddFolder("Сообщения:")
...
bm:AddButton("Сохранить", function()
  ...
  --Webhook Settings
local webhookToggle = webhookTab:AddSwitch("Уведомления вебхука в дискорд", function(bool)
	getgenv().settings.webhookToggle = bool
	saveSettings()
end)
webhookToggle:Set(getgenv().settings.webhookToggle)

local serverHopDonation = webhookTab:AddSwitch("Уведомление после смены сервера", function(bool)
	if settingsLock then
		return
	end
...
local PingEveryoneHighDono = webhookTab:AddSwitch("Пинг всех при минимальном донате", function(bool)
	if settingsLock then
		return 
	end 
...
webhookTab:AddButton("Тест", function()
	if getgenv().settings.webhookBox then
		oldWebhook("Вебхук работает!")
	end
end)

 -- looks better

webhookTab:AddLabel('Тип вебхука: ')
...
local TB = webhookTab:AddTextBox("Минимальная сума доната для пинга", function(text)
...
local alhop = serverHopTab:AddSwitch("Рандомная смена сервера", function(bool)
	if settingsLock then
		return
	end
...
local sHopSwitch = serverHopTab:AddSwitch('Смена сервера после доната', function(bool)
	if settingsLock then
		return
	end
...
local staffHopSwitch = serverHopTab:AddSwitch('Смена сервера если админы', function(bool)
	if settingsLock then
		return
	end
...
local taggedBoothHopSwitch = serverHopTab:AddSwitch('Смена сервера если цензура стойки', function(bool)
	if settingsLock then
		return
	end
...
local gSHSwitch = serverHopTab:AddSwitch('Смена сервера если цель достигнута', function(bool)
	if settingsLock then
		return
	end
...
local serverHopMinAmount = serverHopTab:AddTextBox("Минимум задоначено", function(text)
...
local gSHAmount = serverHopTab:AddTextBox("Цель смены сервера", function(text)
...
serverHopTab:AddLabel("Таймер перезапускается после захода на сервер")
...
local autoReply = otherTab:AddSwitch("Авто-ответчик [БЕТА]", function(bool)
	if settingsLock then
		return
	end
...
local spinToggle = otherTab:AddSwitch('Вращение [1R$ = +1 speed]', function(bool)
...
local jumpSwitcher = otherTab:AddSwitch('1R$ = +1 сила прыжка', function(bool)
	if settingsLock then
		return
	end
...
local fpsBoosts = otherTab:AddSwitch('Удаление карты', function(bool)
...
local jumpsPerRB = otherTab:AddSlider("Прыжок за робуксы", function(x)
	if settingsLock then
		return
	end
...
local spinMultiplier = otherTab:AddSlider("Множитель скорости вращения", function(x)
	if settingsLock then
		return
	end
...
if setfpscap and type(setfpscap) == "function" then
	local fpsLimit = otherTab:AddSlider("ФПС Лимит", function(x)
		if settingsLock then
			return
		end
...
otherTab2:AddLabel("Позиция у стойки:")
...
otherTab2:AddLabel("Авто-ответчик")
otherTab2:AddLabel("Отвечает на 'Привет'")
...
otherTab2:AddLabel("Отвечает на 'Ты бот'")
local BotResponce = otherTab2:AddConsole({
	["y"] = 40,
	["source"] = "",
})
otherTab2:AddLabel("Отвечает на 'Пж задонать'")
local hfull = ''
...
otherTab2:AddLabel("Отвечает на 'Ты скамер'")
		...
		if playerWhoDonated then
			if getgenv().settings.webhookType == 'New' then
			 pcall(function()
			  ...
			  pcall(function()
				oldWebhook(Players.LocalPlayer.Name .. ' | Сумма доната: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value - RaisedC) .. ' | [После комиссии]: ' .. tostring(math.floor((Players.LocalPlayer.leaderstats.Raised.Value - RaisedC) * 0.6)) .. ' | Всего: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value) .. ' | Донатер: ' .. playerWhoDonated.Name)
			  end)
			end
		else
			if getgenv().settings.webhookType == 'New' then
				...
				pcall(function()
					oldWebhook(Players.LocalPlayer.Name .. ' | Сумма доната: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value - RaisedC) .. ' | [После комиссии]: ' .. tostring(math.floor((Players.LocalPlayer.leaderstats.Raised.Value - RaisedC) * 0.6)) .. ' | Всего: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value))
				end)
			end
		end
	end
...
if getgenv().settings.webhookAfterSH then
    if Players.LocalPlayer.DisplayName ~= Players.LocalPlayer.Name then
        oldWebhook(Players.LocalPlayer.DisplayName .. ' (' .. Players.LocalPlayer.Name .. ') сменил сервер!')
    else
        oldWebhook(Players.LocalPlayer.Name .. ' сменил сервер!')        
    end
end
...
if game:GetService("CoreGui").imgui.Windows.Window.Title.Text == "Загрузка..." then
	game:GetService("CoreGui").imgui.Windows.Window.Title.Text = "*"
end
